...
std::pair<std::string, int> readServerInfo()
{
	...
	{
		std::cout << "erorr reading server info" << std::endl;
		exit(1);
	}	
...
void writeMyInfo(std::string name, std::string uuid, std::string privateKey)
{
	std::ofstream file("my_info.txt");
	if (file.is_open())
	{
		file << name << std::endl;
		file << uuid << std::endl;
		...
		std::cout << "cant open info file " << std::endl;
}
...
bool readMyInfo(std::string& name, ClientID& uuid, std::string& privateKey)
{
	std::string firstLine, secondLine, thirdLine, line;
	std::ifstream file("my_info.txt");
	if (file.is_open())
	{
		file >> firstLine;
		file >> secondLine;
...
int main()
{
	...
	do
	{
		...
		{
			// check my.info already exists
			bool isUserRegistered = readMyInfo(clientName, clientID, privateKey);
			if (isUserRegistered)
			{
			...
			if (response->GetCode() == 2000)
			{
				...
				writeMyInfo(clientName, std::static_pointer_cast<RegisterSuccessResponse>(response)->ClientIdAsString(), privateKey);
			}
